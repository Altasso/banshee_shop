...
class TestCrudUserUpdate:
    def test_update_user_success(self, sample_user):
        ...
        updated_user = UserCrud.update_user(sample_user.id, update_data)
    ...
    def test_update_user_not_exists(self):
        updated_user = UserCrud.update_user(999999, {"first_name": "Test"})
        assert updated_user is None

    def test_update_user_email_success(self, sample_user):
        new_email = "newemail@example.com"
        updated_user = UserCrud.update_user(sample_user.id, {"email": new_email})
        assert updated_user is not None
    ...
    def test_update_user_email_duplicate(self, sample_user, create_user):
        ...
        with pytest.raises(ValidationError) as exc_info:
            UserCrud.update_user(sample_user.id, {"email": another_user.email})
    ...
    def test_update_user_password(self, sample_user):
        ...
        updated_user = UserCrud.update_user(sample_user.id, {"password": new_password})
    ...
    def test_update_user_password_empty_string(self, sample_user):
        ...
        updated_user = UserCrud.update_user(sample_user.id, {"password": ""})
