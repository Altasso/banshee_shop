...
from users.auth_mixins import AdminRequiredMixin, OwnerOrAdminMixin
from users.forms.user_forms import (
    UserCreateForm,
    UserUpdateForm,
)
...
class UserDetailView(OwnerOrAdminMixin, DetailView):
    model = User
    template_name = "user_templates/user_detail.html"
    context_object_name = "user"

    def get_object(self, queryset=None):
        user = UserCrud.get_user_by_id(self.kwargs.get("pk"))
        if not user:
...
class UserCreateView(AdminRequiredMixin, SuccessMessageMixin, CreateView):
    ...
    template_name = "user_templates/user_form.html"
    success_url = reverse_lazy("user-list")
    success_message = "Пользователь %(email)s успешно создан"

    def form_valid(self, form):
        try:
            user = UserCrud.create_user(form.cleaned_data)
...
class UserUpdateView(OwnerOrAdminMixin, SuccessMessageMixin, UpdateView):
    ...
    def form_valid(self, form):
        try:
            ...
            user = UserCrud.update_user(self.object.id, form.cleaned_data)
            if not user:
...
class UserDeleteView(AdminRequiredMixin, DeleteView):
    model = User
    template_name = "user_templates/user_confirm_delete.html"
    success_url = reverse_lazy("user-list")
    ...
    def delete(self, request, *args, **kwargs):
        ...
        success = UserCrud.delete_user(self.object.id)
        if success:
            messages.success(
